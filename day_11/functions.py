...
def area_of_circle (radius):
    return 'area = %d'%(math.pi * radius * radius)
...
while (month != 'January' and month != 'February' and month != 'March' and month != 'April' and month != 'May' and month != 'June' and month != 'July' and month != 'August' and month != 'September' and month != 'November' and month != 'December'):
    month = input('Not a valid month. Enter a month: ')

print(f'The season is {check_season(month)}')

# Write a function called calculate_slope which return the slope of a linear equation
def calculate_slope (y_var, x_var, b_var):
    return (y_var - b_var) / x_var

print(calculate_slope(3,4,5))

# Quadratic equation is calculated as follows: ax² + bx + c = 0. Write a function which calculates solution set of a quadratic equation, solve_quadratic_eqn
def solve_quadratic_eqn (a_var, b_var, c_var):
    a = a_var / a_var
    b = b_var / a_var
    c = -c_var / a_var
    sq_coef = (b / 2) ** 2
    c = c + sq_coef
    x1 = -math.sqrt(sq_coef) + c
    x2 = -math.sqrt(sq_coef) - c
    return x1, x2
...
def reverse_list (forward_list):
    ...
    for item in range(len(forward_list) - 1, 0, -1):
        reversed_list.append(item)

    return reversed_list


reversed_list = reverse_list([0,1,2,3])
print(reversed_list)

# Declare a function named capitalize_list_items. It takes a list as a parameter and it returns a capitalized list of items
def capitalize_list_items (old_list):
    new_list = old_list.copy()

    for num in range(0, len(new_list)):
        if isinstance(new_list[num], str):
            new_list[num] = new_list[num].upper()
...
print(f'Sum of numbers: {sum_of_numbers(4)}')
...
print(f'Sum of odds: {sum_of_odds(6)}')
...
print(f'Sum of evens: {sum_of_even(8)}')
...
print(f'Number of evens: {evens_and_odds(10)[0]}, number of odds: {evens_and_odds(10)[1]}')
...
def calculate_median (arr):
    ...
    if arr_length % 2 == 0:
        median = (sorted_arr[arr_length // 2] + sorted_arr[(arr_length // 2) - 1])
        return median
    else:
...
def calculate_variance (arr):
    ...
    for num in arr:
        differences.append((num - mean) ** 2)

    return sum(differences) / len(arr)
...
print(calculate_mode([1,1,2,2,2,3,4]))
...
number = input('Enter a number to see if it is prime: ')

while isinstance(number, int) == False:
    try:
        number = int(number)
    except:
        number = input('Not a valid number, enter a number: ')
...
def same_data_type (list):
    ...
    for item in list:
        if isinstance(item, data_type) == False:
            return False
...
def is_python_variable (var):
    if var[0].isalpha() == False and var[0] != '_':
        return False
...
print(is_python_variable('snake_case'))
...
def most_spoken_languages ():
    ...
    for country in countries_data:
        for country_language in country['languages']:
            language_exists = False

            for language in languages:
                if language.get('name') == country_language:
    ...
    languages.sort(key=sort_by_count, reverse=True)
    names = list()
...
def sort_by_pop (item):
    return item['population']

def most_populated_countries ():
    countries = countries_data.copy()
    countries.sort(key=sort_by_pop, reverse=True)
    names = list()
